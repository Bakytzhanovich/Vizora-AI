    ...
    <div className="min-h-screen bg-bg px-4 py-10">
      <div className="max-w-5xl mx-auto">
        <button
          ...
          className="flex items-center gap-1.5 text-secondary hover:text-primary text-sm mb-6 transition-colors"
        >
          <ArrowLeft size={16} />
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl sm:text-4xl font-bold text-primary mb-2">{t("hero_title")}</h1>
          <p className="text-secondary">{t("hero_subtitle")}</p>
        </div>
          ...
          <div className="inline-flex bg-card border border-border rounded-xl p-1">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${
                billingPeriod === "monthly" ? "bg-accent text-white" : "text-secondary"
              }`}
            >
              {t("toggle_monthly")}
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${
                billingPeriod === "yearly" ? "bg-accent text-white" : "text-secondary"
              }`}
            >
              {t("toggle_yearly")}
            </button>
        ...
        {error && (
          <div className="max-w-md mx-auto mb-6 text-center text-sm text-error">{error}</div>
        )}
          ...
          <div className="grid md:grid-cols-3 gap-5 mb-8">
            {plans.map((plan) => {
              ...
              return (
                ...
                >
                  {isPopular && (
                    <span className="absolute -top-3 left-1/2 -translate-x-1/2 flex items-center gap-1 text-xs font-bold px-3 py-1 rounded-full bg-accent text-white">
                  ...
                  <div className="mb-5">
                    {isFree ? (
                      <>
                        ...
                        <div className="text-secondary text-xs mt-0.5">{t("forever")}</div>
                      </>
                    ) : billingPeriod === "yearly" ? (
                      <>
                        <span className="text-3xl font-bold text-primary">{formatKzt(monthlyEquivalent ?? 0)}</span>
                        <span className="text-secondary text-sm">{t("per_month")}</span>
                        <div className="text-secondary text-xs mt-0.5">{formatKzt(price)}{t("per_year")}</div>
                      </>
                    ) : (
                  ...
                  <ul className="space-y-2.5 mb-6 flex-1">
                    {featureRows(plan).map((row, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm">
                        ...
                        <span className={row.ok ? "text-primary" : "text-secondary"}>{row.label}</span>
                      </li>
                    ))}
                  </ul>
                  <button
                    ...
                    className={`w-full py-3 rounded-xl text-sm font-semibold transition-colors disabled:opacity-60 ${
                      isPopular
                        ...
                        : "bg-border hover:bg-border-hover text-primary"
                    }`}
                  >
        ...
        {standardMonthly != null && (
          <div className="max-w-2xl mx-auto mb-10 rounded-2xl border border-accent/30 bg-accent/5 p-5 flex items-start gap-3">
            <Lightbulb size={20} className="text-accent shrink-0 mt-0.5" />
            <div>
              <p className="text-primary font-semibold text-sm mb-1">{t("value_box.title")}</p>
        ...
        {/* Social proof */}
        {socialProof && (
          <div className="flex flex-wrap justify-center gap-x-6 gap-y-2 text-sm text-primary mb-4">
        ...
        {/* Trust row */}
        <div className="flex flex-wrap justify-center gap-x-6 gap-y-2 text-xs text-secondary mb-14">
        ...
        <div className="max-w-2xl mx-auto">
          <h2 className="text-xl font-bold text-primary mb-5 text-center">{t("faq_title")}</h2>
          <div className="space-y-3">
            {(t("faq", { returnObjects: true }) as { q: string; a: string }[]).map((item, i) => (
              <div key={i} className="rounded-xl border border-border bg-card p-4">
                <p className="text-primary font-semibold text-sm mb-1.5">{item.q}</p>
