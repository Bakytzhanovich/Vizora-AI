    ...
    <div className="fixed inset-0 z-50 bg-bg flex items-center justify-center px-4">
      <div className="max-w-sm w-full text-center">
        <div className="w-14 h-14 rounded-full bg-accent/15 flex items-center justify-center mx-auto mb-4">
          <Timer size={26} className="text-accent" />
        </div>
        <h1 className="text-xl font-bold text-primary mb-2">{t("trial_ended.title")}</h1>
        <p className="text-secondary text-sm mb-1">{t(answeredKey, { count: answered })}</p>
        <p className="text-secondary text-sm mb-6">{t("trial_ended.real_interview_note", { count: total })}</p>

        <p className="text-primary text-sm font-semibold mb-3">{t("trial_ended.continue_with_standard")}</p>

        <button
          onClick={() => router.push("/pricing")}
          className="w-full bg-gradient-to-r from-accent to-accent-light text-white font-bold py-4 rounded-xl shadow-lg shadow-accent/20 text-sm transition-transform hover:scale-[1.01] mb-3"
        >
          {t("trial_ended.subscribe_cta")}
        </button>
        <button
          onClick={onSeeResults}
          className="w-full text-secondary hover:text-primary text-sm font-medium py-2 transition-colors"
        >
